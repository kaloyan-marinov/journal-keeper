...
4. ensure that running the tests results in a PASS by issuing any one of the following:
...
5. create an empty SQLite database and apply all database migrations by issuing one of the following:
...
6. verify that the previous step was successful by issuing `$ sqlite3 <the-value-of-DATABASE_URL-in-your-.env-file>` and then issuing:
...
7. launch a terminal instance and, in it, start a process responsible for serving the application instance; the ways of starting such a process can be broken down into the following categories:
...
9. launch another terminal window and, in it, issue the following requests:
